<?php 
	trait LaptopModel
	{
		public function modelRead($recordPerPage)
		{
			...
			$query=$conn->query("select * from Loaisp where name = 'Lap top' or name = 'Mac book' order by id asc limit $from,$recordPerPage");
		...
		{
			$conn=Connection::getInstance();
			$query=$conn->query("select id from Loaisp where name = 'Lap top' or name = 'Mac book'");
		...
		public function modelListParent($id)
		{
			...
			$query=$conn->query("select loaisp.id,nhasanxuat.name as 'nsx',loaisp.name as 'loaisp' from nhasanxuat,loaisp WHERE loaisp.id <> $id and loaisp.id_nsx=nhasanxuat.id AND loaisp.parent_id=0 and loaisp.name='Lap top'");
			return $query->fetchAll();
		}
	}
