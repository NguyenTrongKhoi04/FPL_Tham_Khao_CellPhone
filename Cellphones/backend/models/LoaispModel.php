<?php 
	trait LoaispModel
	{
		public function modelRead($recordPerPage)
		{
			...
			$query=$conn->query("select * from Loaisp where name like '%Điện thoại%' order by id asc limit $from,$recordPerPage");
		...
		{
			$conn=Connection::getInstance();
			$query=$conn->query("select id from Loaisp where name like '%Điện thoại%'");
			...
			// 	$query1=$conn->query("select id from Loaisp where parent_id = $value->id");
			// 	$num2=$num2+$query1->rowCount();
			// }
			// return $num1+$num2;
		...
		public function modelListLoaisp()
		{
			$conn=Connection::getInstance();
		...
		public function modelListParent($id)
		{
			...
			$query=$conn->query("select loaisp.id,nhasanxuat.name as 'nsx',loaisp.name as 'loaisp' from nhasanxuat,loaisp WHERE loaisp.id <> $id and loaisp.id_nsx=nhasanxuat.id AND loaisp.parent_id=0 and loaisp.name='Điện thoại'");
			return $query->fetchAll();
		}
		...
		public function postDetailCreateModel()
		{

			$nameNsx=$_POST['id_nsx'];
			$kichthuoc=$_POST['kichthuoc'];
			$trongluong=$_POST['trongluong'];
			$ram=$_POST['ram'];
			$rom=$_POST['rom'];
			$hedieuhanh=$_POST['hedieuhanh'];
			$loaiman=$_POST['loaiman'];
			$kichthuocman=$_POST['kichthuocman'];
			$dophangiai=$_POST['dophangiai'];
			$sosim=$_POST['sosim'];
			$id=isset($_GET['id'])?$_GET['id']:"";

			$conn=Connection::getInstance();
			$conn->query("insert into detail set id_products=$id,hang='$nameNsx',kichthuoc='$kichthuoc',trongluong='$trongluong',ram='$ram',rom='$rom',hedieuhanh='$hedieuhanh',loaiman='$loaiman',kichthuocman='$kichthuocman',dophangiai='$dophangiai',sosim='$sosim'");

			return true;
		}
		public function postDetailUpdateModel()
		{
			$nameNsx=$_POST['id_nsx'];
			$kichthuoc=$_POST['kichthuoc'];
			$trongluong=$_POST['trongluong'];
			...
			$id=isset($_GET['id'])?$_GET['id']:"";
			$conn=Connection::getInstance();
			$conn->query("update detail set id_products=$id,hang='$nameNsx',kichthuoc='$kichthuoc',trongluong='$trongluong',ram='$ram',rom='$rom',hedieuhanh='$hedieuhanh',loaiman='$loaiman',kichthuocman='$kichthuocman',dophangiai='$dophangiai',sosim='$sosim'");
		...
		public function exportExcelModel()
		{
			...
			$excel->getActiveSheet()->setCellValue('B1', 'Tên nhà sản xuất');
			$excel->getActiveSheet()->setCellValue('C1', 'Tên sản phẩm');
			$excel->getActiveSheet()->setCellValue('D1', 'Số lượng');
			$excel->getActiveSheet()->setCellValue('E1', 'Giá');
			$excel->getActiveSheet()->setCellValue('F1', 'Giảm giá');

			$conn=Connection::getInstance();
			$query=$conn->query("select s.id,n.name,s.name as namesp,s.slsp,s.price,s.discount
			from Loaisp s 
			JOIN  nhasanxuat n on s.id_nsx=n.id
			WHERE s.parent_id in (select id from loaisp WHERE name like '%Điện thoại%' and parent_id=0) 
			GROUP by s.id");
			$result=$query->fetchAll();
			...
			foreach ($result as $row) {
			    ...
			    $excel->getActiveSheet()->setCellValue('C' . $numRow, $row->namesp);
			    $excel->getActiveSheet()->setCellValue('D' . $numRow, $row->slsp);
			    $excel->getActiveSheet()->setCellValue('E' . $numRow, $row->price);
			    $excel->getActiveSheet()->setCellValue('F' . $numRow, $row->discount);
			    $numRow++;
			}
